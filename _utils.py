...
def prepare_centroids(inputs, labels):
    avaliable_labels = (labels.sum(dim=0) > 0).nonzero().squeeze()
    idxes_per_class = {}
    for i in avaliable_labels:
        idxes = (labels[:, i] == 1).nonzero().squeeze()
        idxes_per_class[i] = idxes

    _inputs = []
    _labels = []
    for k, v in idxes_per_class.items():
        _inputs.append(inputs[v].mean(0))

        centroid_label = labels[v].mean(0)
        centroid_label = torch.where(centroid_label > 0.5, 1.0, 0.0)
        _labels.append(centroid_label)
    _inputs = torch.stack(_inputs, dim=0).sign()
...
def validate(args, query_loader, dbase_loader, early_stopping, epoch, **kwargs):
    ...
    # map_v_10 = map_fnc(qB, rB, qL, rL, None)
    # map_v_30 = map_fnc(qB, rB, qL, rL, 300)
    # map_v_50 = map_fnc(qB, rB, qL, rL, 500)
    #
    # p10 = precision_at_k(qB, rB, qL, rL, topk=100)
    # p30 = precision_at_k(qB, rB, qL, rL, topk=300)
    # p50 = precision_at_k(qB, rB, qL, rL, topk=500)

    # r10 = recall_at_k(qB, rB, qL, rL, topk=10)
    # r30 = recall_at_k(qB, rB, qL, rL, topk=30)
    # r50 = recall_at_k(qB, rB, qL, rL, topk=50)
    # r10 = calc_recall_at_k(qB, rB, qL, rL, k=1)
    # r30 = calc_recall_at_k(qB, rB, qL, rL, k=3)
    # r50 = calc_recall_at_k(qB, rB, qL, rL, k=5)
    #
    # print(f'map_v_10:{map_v_10}, map_v_20:{map_v_30}, map_v_30:{map_v_50}')
    # print(f'p10:{p10}, p20:{p30}, p30:{p50}')
    # print(f'r1:{r10}, r2:{r30}, r3:{r50}')

    rB, rL = prepare_centroids(rB, rL)
    map_v1 = map_fnc(qB, rB, qL, rL, args.topk)
    # map_v_10 = map_fnc(qB, rB, qL, rL, 100)
    # map_v_30 = map_fnc(qB, rB, qL, rL, 300)
    # map_v_50 = map_fnc(qB, rB, qL, rL, 500)
    #
    # p10 = precision_at_k(qB, rB, qL, rL, topk=100)
    ...
    # if epoch %10 == 0:
    # save_mat(args, qB, rB, qL, rL, epoch)
    ...
    logger.info(
        f"[Evaluating][dataset:{args.dataset}][bits:{args.n_bits}][epoch:{epoch}/{args.n_epochs - 1}][best-mAP{map_k}:{map_o:.4f}][mAP{map_k}:{map_v:.4f}][mAP_Centroid{map_k}:{map_v1:.4f}][count:{early_stopping.counter}]"
    )
    return early_stopping.early_stop


def save_mat(args, query_img, retrieval_img, query_labels, retrieval_labels, epoch, mode_name="i2t"):
    save_dir = os.path.join(args.save_dir, "PR_cruve")
    ...
    retrieval_labels = retrieval_labels.cpu().numpy()
    ...
    scio.savemat(os.path.join(save_dir + "-ours-" + str(epoch) + args.dataset + ".mat"), result_dict)
    print(f'PR Save Successfully')
...
def precision_at_k(qB, rB, qL, rL, topk=10):
    """
    Calculate mean Precision@k.

    Args:
        qB (torch.Tensor): Query hash codes, shape [N, K]
        rB (torch.Tensor): Retrieval hash codes, shape [M, K]
        qL (torch.Tensor): Query labels, shape [N] or [N, C]
        rL (torch.Tensor): Retrieval labels, shape [M] or [M, C]
        topk (int): top-k retrieved samples

    Returns:
        torch.Tensor: mean Precision@k
    """
    N, K = qB.shape
    M = rB.shape[0]
    topk = min(topk, M)
    ...
    # relevance mask
    if qL.ndim == 1:
        masks = qL.unsqueeze(1) == rL.unsqueeze(0)   # [N, M]
    elif qL.ndim == 2:
        masks = (qL @ rL.T) > 0                      # [N, M]
    else:
    ...
    # Hamming distance
    hamming_dists = 0.5 * (K - qB @ rB.T)           # [N, M]

    precision_sum = hamming_dists.new_tensor(0.0)

    for i in range(N):
        hamming_dist = hamming_dists[i]
        topk_indices = torch.topk(hamming_dist, topk, largest=False).indices
        retrieval = masks[i, topk_indices].float()  # [topk]

        precision_sum += retrieval.mean()

    return precision_sum / N

def calc_recall_at_k(qB, rB, query_L, retrieval_L, k=1, rank=0):
    """
    计算 Recall@k（按你当前 Recall@1 的口径扩展）
    即：对每个 query，只要 top-k 结果中至少有一个相关样本，就记为 1。

    Args:
        qB (torch.Tensor): 查询样本哈希码, shape [num_query, bit]
        rB (torch.Tensor): 检索库哈希码, shape [num_retrieval, bit]
        query_L (torch.Tensor): 查询标签, shape [num_query, num_classes] 或 [num_query]
        retrieval_L (torch.Tensor): 检索库标签, shape [num_retrieval, num_classes] 或 [num_retrieval]
        k (int): top-k
        rank (int): 保留参数，当前未使用
    ...
    if query_L.is_cuda:
        query_L = query_L.cpu()
    if retrieval_L.is_cuda:
        retrieval_L = retrieval_L.cpu()

    num_retrieval = rB.shape[0]
    k = min(k, num_retrieval)

    # FAISS 需要 float32
    qB_np = qB.detach().float().numpy()
    rB_np = rB.detach().float().numpy()

    # 建立索引（沿用你原来的 L2）
    faiss_search_index = faiss.IndexFlatL2(rB.shape[-1])
    faiss_search_index.add(rB_np)

    # 搜索 top-k
    _, k_nn = faiss_search_index.search(qB_np, k)   # [num_query, k]

    recall_at_k = 0

    for i in range(len(query_L)):
        query_label = query_L[i]
        retrieved_labels = retrieval_L[k_nn[i]]     # [k, num_classes] 或 [k]

        if query_L.ndim == 2:
            # 多标签：top-k 中只要有一个样本与 query 共享至少一个标签
            hit = ((retrieved_labels * query_label).sum(dim=1) > 0).any()
        elif query_L.ndim == 1:
            # 单标签：top-k 中只要有一个样本标签相同
            hit = (retrieved_labels == query_label).any()
        else:
            raise NotImplementedError(f"Not support label shape: {query_L.shape}")

        if hit:
            recall_at_k += 1

    recall_at_k = recall_at_k / len(query_L)
    return recall_at_k

def recall_at_k(qB, rB, qL, rL, topk=10):
    """
    ...
    recall_sum = hamming_dists.new_tensor(0.0)

    for i in range(N):
        total_relevant = masks[i].sum().float()     # total positives in database
        if total_relevant == 0:
            continue

        hamming_dist = hamming_dists[i]
        topk_indices = torch.topk(hamming_dist, topk, largest=False).indices
        retrieval = masks[i, topk_indices].float()

        retrieved_relevant = retrieval.sum()
        recall_sum += retrieved_relevant / total_relevant
